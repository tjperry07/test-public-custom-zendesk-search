...
export const bestHitTemplate = ({
  ...
  const title = Array.isArray(titleAttribute)
    ? titleAttribute.reduce((acc, attr) => acc[attr], item)
    : item[titleAttribute];
  const formattedTitleAttribute = Array.isArray(titleAttribute)
    ? titleAttribute.join('.')
    : titleAttribute;
  return (
    <a
      data-testid="best-hit"
      href={`${BASE_URLS[item.sourceId]}${item.url}#:~:text=${highlight}`}
      className={stl`p-4 pt-3 border-2 border-nebula-500 rounded no-underline hover:no-underline flex justify-between items-center usercontent`}
    >
      <div style={{ width: '90%' }}>
        <span
          className={`DocSearch-Hit-title ${stl`text-grey-500 uppercase font-sans font-bold text-base block mb-1`}`}
        >
          {item.sourceId}
        </span>
        <h4
          className={`DocSearch-Hit-title ${stl`text-lg text-grey-900 font-bold mb-4`}`}
          dangerouslySetInnerHTML={{
            __html:`${item._snippetResult.hasOwnProperty('title') ? item._snippetResult.title.value : item.title}`}}
        />
        {item._answer.extractAttribute === formattedTitleAttribute ? (
          <p
            className={`DocSearch-Hit-title ${stl`text-md text-grey-600 leading-base`}`} dangerouslySetInnerHTML={{
              __html: `${contentAttribute==="body_safe"?item._snippetResult.body_safe.value:contentAttribute==="description"?item._snippetResult.description.value:contentAttribute==="content"?item._snippetResult.content.value:"..."}`,
            }}
          />

        ) : (
          <p
            className={`DocSearch-Hit-title ${stl`text-md text-grey-600 leading-base`}`}
            dangerouslySetInnerHTML={{
              __html: `...${item._answer.extract}...`,
            }}
          />
        )}
      </div>
